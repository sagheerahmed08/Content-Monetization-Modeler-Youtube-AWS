...
if df is not None:
    ...
    num_features = [
        "views", "comments", "video_length_minutes", "subscribers",
...
with tab1:
    ...
    if st.button("🧠 Train & Upload All Models"):
        with st.spinner("Training models... ⏳"):
            for name, model in models.items():
                ...
                # Cross-validation
                scores = cross_val_score(pipe, X_train, y_train, scoring="r2", cv=5)
            ...
            best_model = load_model_from_s3(S3_BUCKET, f"{MODEL_PREFIX}/{best_name}.joblib")
            ...
            st.success("🎉 Training completed and results.csv uploaded!")
...
with tab2:
    ...
    if df is not None:
        try:
            ...
            st.warning("⚠️ No results.csv found, creating empty results table.")
            df_results = pd.DataFrame(columns=["Model", "CV_R2_Mean", "CV_R2_STD"])
        ...
        for idx, file_name in enumerate(model_files):
            ...
            try:
                model = load_model_from_s3(S3_BUCKET, f"{MODEL_PREFIX}/{file_name}")
